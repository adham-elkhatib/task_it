...
class RTDBRepo<T> {
  ...
  RTDBRepo({required this.path, required this.discardKey});
  ...
  //SECTION Create
  //t1 Create
  Future create(T item, {String? key, bool generateKey = true}) async {
    //
    assert(
      generateKey || key != null,
    ...
    try {
      ...
      ErrorHandlingService.handle(e, s, location: 'RTDBRepo<$T>/create');
    }
  }
  ...
  Future createMultiple(
    ...
    //
    assert(
      generateKey || (keys != null && keys.length == items.length),
  ...
  Future<T?> read(String key) async {
    try {
      ...
      ErrorHandlingService.handle(e, s, location: 'RTDBRepo<$T>/read');
      return null;
    }
  }
  ...
  Future<List<T?>?> readAll() async {
    try {
      ...
      ErrorHandlingService.handle(e, s, location: 'RTDBRepo<$T>/readAll');
      return null;
    }
  }
    ...
    try {
      if (!isTransact) {
        ...
        TransactionResult? result = await _db.transact(
          ...
          (currentData) {
            if (currentData != null) {
              return fromModel(transactionProcess!(toModel(currentData) as T));
            } else {
      ...
      ErrorHandlingService.handle(e, s, location: 'RTDBRepo<$T>/update');
      return false;
    }
  }
  ...
  Future<bool> updateMultible(
    ...
    //
    assert(
      itemKeys.length == updatedItems.length,
    ...
    try {
      ...
      return _db
          .updateMultible(
            ...
            Map.fromIterables(
              itemKeys.map((e) => e),
              updatedItems.map((e) => fromModel(e)),
            ),
          )
          .then((value) => true);
      //
